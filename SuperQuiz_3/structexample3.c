 ...
 * deletion of students. [Not that we don't actually use those wonderful
...
bool comesBefore(const Student* student1, const Student* student2)
{
	if (strcmp(student1->name, student2->name) < 0){
		return 1;
	} else if  (strcmp(student1->name, student2->name) == 0 && student1->age < student2->age){
		return 1;
	}
	return 0;
}
...
Student* insert(Student* student, Student* list)
{
	...
	if (comesBefore(student, list)){
		...
		prev= list;
		current = list->next;
		while (current != NULL && comesBefore(current, student)){
			prev = current;
